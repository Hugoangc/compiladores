...
// int obterValor(Operando operando)
// {
//   if (holds_alternative<int>(operando))
//   {
//     return get<int>(operando);
//   }
//   else if (holds_alternative<string>(operando))
//   {
//     string var = get<string>(operando);
//     if (variaveis.find(var) == variaveis.end())
//     {
//       throw runtime_error("Variável não definida: " + var);
//     }
//     return variaveis[var]; // Aqui, a variável 'numBloco_0' será mapeada para o valor que ela contém
//   }
//   else
//   {
//     throw runtime_error("Operando inválido.");
//   }
// }
...
void executarAritmetica(const Instrucao &instrucao)
{
  ...
  // int v2 = (operador == "+" || operador == "-") ? 0 : obterValor(operando2);
...
void executarRelacional(const Instrucao &instrucao)
{
  ...
  if (holds_alternative<string>(operando1))
  {
    ...
    operando1 = variaveis[var1]; // Aqui você resolve para o valor da variável
  }

  if (holds_alternative<string>(operando2))
  {
    string var2 = get<string>(operando2);
    operando2 = variaveis[var2]; // Aqui você resolve para o valor da variável
  }
...
void executarIf(const Instrucao &instrucao)
{
  ...
  // Avaliar a condição e atualizar o ponteiro
  // if (valorCondicao) // Condição verdadeira
  // {
  //   if (labels.find(label1) == labels.end())
  //   {
  //     throw runtime_error("Label nao encontrado: " + label1);
  //   }
  //   ponteiro = labels[label1] - 1; // Ajuste do ponteiro
  // }
  // else // Condição falsa
  // {
  //   if (labels.find(label2) == labels.end())
  //   {
  //     throw runtime_error("Label nao encontrado: " + label2);
  //   }
  //   ponteiro = labels[label2] - 1; // Ajuste do ponteiro
  // }
  if (valorCondicao) // Condição verdadeira
  {
...
{
  string label = get<string>(get<1>(instrucao));
  if (labels.find(label) == labels.end())
  {
    throw runtime_error("Label nao encontrado: " + label);
  }
  ponteiro = labels[label] - 1;
}
...
void executarCall(const Instrucao &instrucao)
{
  ...
  if (comando == "PRINT")
  {
    if (holds_alternative<int>(operando1))
    {
      cout << "Saida: " << get<int>(operando1) << endl;
    }
    else
    {
      string var = get<string>(operando1);
      if (variaveis.find(var) != variaveis.end())
      {
        cout << "Saida: " << variaveis[var] << endl;
      }
      else
      {
        cout << "Saida: " << var << endl;
  ...
  else if (comando == "SCAN")
  {
    // if (!holds_alternative<string>(operando1))
    // {
    //   throw runtime_error("SCAN requer um nome de variavel no terceiro operando.");
    // }
    // string var = get<string>(operando1);
    // int valor;
    // cout << "Entrada para " << var << ": ";
    // cin >> valor;
    // variaveis[var] = valor;
...
int main()
{
  try
  {
    carregarTuplas("text2.txt"); // Substituir pelo caminho correto
    carregarLabels();
    executar();
  }
